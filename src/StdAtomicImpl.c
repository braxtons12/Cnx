#include <C2nxt/StdPlatform.h>
	...
	#define __IS_X86                                    \
		(IS_STD_PLATFORM_ARCHITECTURE(STD_PLATFORM_X86) \
		 || IS_STD_PLATFORM_ARCHITECTURE(STD_PLATFORM_X86_64))

	#define __IS_ARM IS_STD_PLATFORM_ARCHITECTURE(STD_PLATFORM_ARM64)
...
void __atomic_store64(volatile atomic_i64* object, i64 value) {
	#if __IS_X86 && !IS_STD_PLATFORM_ARCHITECTURE(STD_PLATFORM_X86_64) // is 32bit x86
	__compiler_barrier();
	__iso_volatile_store64(object, value);
	...
	__memory_barrier();
	#endif // __IS_X86 && !IS_STD_PLATFORM_ARCHITECTURE(STD_PLATFORM_X86_64)
}
...
i64 __atomic_exchange_explicit64(volatile atomic_i64* object,
								 ...
								 i64 desired,
								 [[maybe_unused]] memory_order order) {
	#if __IS_X86 && !IS_STD_PLATFORM_ARCHITECTURE(STD_PLATFORM_X86_64) // 32bit x86
	i64 val = __atomic_load64(object);
...
bool __atomic_compare_exchange_strong_explicit8(
	...
	*expected = res;
	std_memcpy(typeof(res), expected, &res, 1);
	return false;
}
